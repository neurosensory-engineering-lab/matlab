...
function [PC1, PC2, PC3, score] = PCA_analysis_for_FUS(all_trials, num_pre_trials, num_post_trials, excluded_trials, initial_stim_frame, max_analysis_frame)
...
%% ----- Normalized Vector Changes and Bar Chart ----- %

% Define the range of frames for pre- and post-FUS trials
valid_trials = setdiff(1:(num_pre_trials + num_post_trials), excluded_trials);
frames_per_trial = max_analysis_frame - initial_stim_frame + 1;

% Collect pre- and post-FUS frames
pre_fus_frames = [];
post_fus_frames = [];
for trial = valid_trials
    frame_start = (trial - 1) * 100 + initial_stim_frame;
    frame_end = frame_start + frames_per_trial - 1;

    if trial <= num_pre_trials
        pre_fus_frames = [pre_fus_frames; score(frame_start:frame_end, 1:3)];
    else
        post_fus_frames = [post_fus_frames; score(frame_start:frame_end, 1:3)];
    end
end

% Calculate centroids
pre_fus_centroid = mean(pre_fus_frames, 1);
post_fus_centroid = mean(post_fus_frames, 1);

% Normalize centroids
pre_fus_norm = pre_fus_centroid / norm(pre_fus_centroid);
post_fus_norm = post_fus_centroid / norm(post_fus_centroid);

% Changes in normalized vector
vector_change = post_fus_norm - pre_fus_norm;

% Console Output
fprintf('Normalized Vector Changes:\n');
fprintf('Pre-FUS Centroid (Normalized): [%.4f, %.4f, %.4f]\n', pre_fus_norm);
fprintf('Post-FUS Centroid (Normalized): [%.4f, %.4f, %.4f]\n', post_fus_norm);
fprintf('Change in Vector: [%.4f, %.4f, %.4f]\n', vector_change);

% Visualize normalized vectors in PCA space
figure;
quiver3(0, 0, 0, pre_fus_norm(1), pre_fus_norm(2), pre_fus_norm(3), 'b', 'LineWidth', 2);
hold on;
quiver3(0, 0, 0, post_fus_norm(1), post_fus_norm(2), post_fus_norm(3), 'r', 'LineWidth', 2);
line([pre_fus_norm(1), post_fus_norm(1)], [pre_fus_norm(2), post_fus_norm(2)], ...
     [pre_fus_norm(3), post_fus_norm(3)], 'Color', 'k', 'LineWidth', 2);
xlabel('PC1');
ylabel('PC2');
zlabel('PC3');
title('Normalized Vector Changes in PCA Space');
legend({'Pre-FUS', 'Post-FUS', 'Change'}, 'Location', 'Best');
grid on;

% % Bar Chart for Average Changes in PC1, PC2, PC3
% average_changes = mean(post_fus_frames - pre_fus_frames, 1);
% normalized_changes = average_changes / sum(abs(average_changes));
% 
% figure;
% bar(normalized_changes, 'FaceColor', 'flat');
% xlabel('Principal Component');
% ylabel('Normalized Change');
% title('Normalized Average Changes in PC1, PC2, PC3');
% xticks(1:3);
