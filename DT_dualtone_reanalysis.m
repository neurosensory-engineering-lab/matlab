...
% 4. PAIRWISE REANALYSIS: Evaluates whether local FUS gain acts as a blind spatial 
%    blanket or varies depending on the acoustic driving frequency.
% 5. CLUTTER-FREE PLOTTING: Removes trend lines and displays brackets ONLY if p < 0.05.
% ========================================================================
...
for idx = 1:num_exps
    ...
    if ~exist(var10, 'var') || ~exist(var30, 'var')
        fprintf('  [!] Missing upscaled PC1 reference maps in workspace for %s\n', baseID);
        continue;
    end
